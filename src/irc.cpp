...
// Copyright (c) 2011-2012 MavroCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("MavroCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MavroCoinTEST0\r");
            Send(hSocket, "WHO #MavroCoinTEST0\r");
        } else {
            // randomly join #MavroCoin00-#MavroCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MavroCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MavroCoin%02d\r", channel_number).c_str());
        }
