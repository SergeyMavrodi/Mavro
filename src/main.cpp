...
uint256 hashGenesisBlock("0x00000550d3933b6a2047ca1676aad590ce72ec1e89a4c63d52c28d8d0fc6eb37");
...
const string strMessageMagic = "MavroCoin Signed Message:\n";
...
{
    // MavroCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // MavroCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mavro genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1486754561;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1195806;

        if (fTestNet)
        {
        	block.nTime    = 1486754622;
            block.nNonce   = 773835;
        }
	    ...
	    assert(block.hashMerkleRoot == uint256("0x795b281dcb431a76034e0a3357b46a9112c2db5f0e3281d1f8e56634fe2996ec"));
        block.print();
		if (!fTestNet) {
...
unsigned char pchMessageStart[4] = { 0xd7, 0x9a, 0xac, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MavroCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MavroCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MavroCoinMiner(CWallet *pwallet)
{
    printf("MavroCoinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MavroCoin-miner");
    ...
    {
        printf("MavroCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MavroCoinMiner, pwallet));
}
